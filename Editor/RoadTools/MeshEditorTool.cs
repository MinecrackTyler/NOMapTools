...
namespace NOMapTools
{
	public class MeshEditorTool
	{
		...
		public MeshEditorTool(RoadTool tool)
		{
			this.tool = tool;
		}
		...
		{
			if (tool.ActiveNetwork == null || selectedPreset == null)
				return;
			...
			foreach (var road in tool.SelectedRoads)
			{
				...
				go.transform.position = RoadTool.GetRoadCenter(road);
		...
		private Mesh GenerateRoadMesh(Road road)
		{
			...
			Vector3 center = RoadTool.GetRoadCenter(road);
